...
PBS_JOBNAME="${PBS_JOBNAME:-gpt2_lora_init_is256}"
MAX_CONCURRENT_RUNS="${MAX_CONCURRENT_RUNS:-2}"
DRY_RUN="${DRY_RUN:-0}"

BASE_OUT="${BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_lora_init_is_256x256_r64_debug}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_lora_init_is_256x256_r64_debug}"
LOG_SUBDIR="${LOG_SUBDIR:-gpt2_lora_init_is_256x256_r64_debug}"
...
echo "Preparing GPT-2 LoRA r=${LORA_RANK} IS 256+256 init comparison"
echo "Output root: ${BASE_OUT}"
echo "Estimator: subset_kl is, head=${HEAD_K}, tail=${TAIL_K}"
echo "Steps: ${NUM_STEPS}; save_every=${SAVE_EVERY}; lr=${LR}"

add_or_skip_run \
  "lora_is256x256_r${LORA_RANK}_default_lora" \
  "${BASE_OUT}/lora_is256x256_r${LORA_RANK}_default_lora" \
  "${COMMON_RUN_KV[@]}" \
  "LORA_INIT=default_lora"

add_or_skip_run \
  "lora_is256x256_r${LORA_RANK}_ridge_svd" \
  "${BASE_OUT}/lora_is256x256_r${LORA_RANK}_ridge_svd" \
  "${COMMON_RUN_KV[@]}" \
  "LORA_INIT=ridge_svd" \
