...
PBS_JOBNAME="${PBS_JOBNAME:-gpt2_ridge_lr_is256}"
MAX_CONCURRENT_RUNS="${MAX_CONCURRENT_RUNS:-2}"
DRY_RUN="${DRY_RUN:-0}"

BASE_OUT="${BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_lora_ridge_lr_schedule_is256_r64_debug}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_lora_ridge_lr_schedule_is256_r64_debug}"
LOG_SUBDIR="${LOG_SUBDIR:-gpt2_lora_ridge_lr_schedule_is256_r64_debug}"
...
echo "Preparing GPT-2 LoRA ridge-init r=${LORA_RANK} IS 256+256 LR schedule sweep"
echo "Output root: ${BASE_OUT}"
echo "Steps: ${NUM_STEPS}; save_every=${SAVE_EVERY}"

for lr in 1.5e-3 3e-3 5e-3; do
  slug="$(lr_slug "${lr}")"
  add_or_skip_run \
    "lora_ridge_is256x256_r${LORA_RANK}_lr${slug}_constant" \
    "${BASE_OUT}/lora_ridge_is256x256_r${LORA_RANK}_lr${slug}_constant" \
    "${COMMON_RUN_KV[@]}" \
    "LR=${lr}" \
    "WARMUP_STEPS=0" \
...
for lr in 3e-3 5e-3 1e-2; do
  slug="$(lr_slug "${lr}")"
  add_or_skip_run \
    "lora_ridge_is256x256_r${LORA_RANK}_lr${slug}_cosine_w10_min0p1" \
    "${BASE_OUT}/lora_ridge_is256x256_r${LORA_RANK}_lr${slug}_cosine_w10_min0p1" \
    "${COMMON_RUN_KV[@]}" \
    "LR=${lr}" \
    "WARMUP_STEPS=10" \
    "LR_SCHEDULE=cosine" \
