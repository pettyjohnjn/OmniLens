...
PBS_JOBNAME="${PBS_JOBNAME:-gpt2_r64_mc}"
...
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_r64_is_250_debug}"
...
echo "Preparing GPT-2 LoRA r=64 importance sampling estimator run"
...
add_or_skip_run \
  "lora_subset_is_r${LORA_RANK}_k${HEAD_K}_tail${TAIL_K}_lr1p5em3" \
  "${BASE_OUT}/lora_subset_is_r${LORA_RANK}_k${HEAD_K}_tail${TAIL_K}_lr1p5em3" \
  "${COMMON_RUN_KV[@]}" \
  "LENS_TYPE=lora" \
  "LOSS_TYPE=subset_kl" \
  "LORA_RANK=${LORA_RANK}" \
...
if (( TOTAL_RUNS == 0 )); then
  echo "No runnable importance sampling run was generated."
  exit 0
fi

MANIFEST_FILE="${STAGING_DIR}/manifest.gpt2_r64_is_250_debug.txt"
printf '%s\n' "${RUN_ENV_FILES[@]}" > "${MANIFEST_FILE}"

echo "Generated ${TOTAL_RUNS} runnable run"
