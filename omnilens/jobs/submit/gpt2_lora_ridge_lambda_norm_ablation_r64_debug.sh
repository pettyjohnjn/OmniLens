...
PBS_WALLTIME="${PBS_WALLTIME:-01:30:00}"
PBS_FILESYSTEMS="${PBS_FILESYSTEMS:-YOUR_PBS_FILESYSTEMS}"
PBS_JOBNAME="${PBS_JOBNAME:-gpt2_ridge_lambda_norm}"
MAX_CONCURRENT_RUNS="${MAX_CONCURRENT_RUNS:-1}"
DRY_RUN="${DRY_RUN:-0}"

BASE_OUT="${BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_lora_ridge_lambda_norm_ablation_r64_debug}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_lora_ridge_lambda_norm_ablation_r64_debug}"
LOG_SUBDIR="${LOG_SUBDIR:-gpt2_lora_ridge_lambda_norm_ablation_r64_debug}"
...
LAMBDAS=(${LAMBDAS:-1e-4 1e-3 1e-2})
NORMALIZATIONS=(${NORMALIZATIONS:-none per_dim_std})
...
add_or_skip_run() {
  ...
  lambda_name="$(lambda_slug "${lambda}")"
  run_name="lora_fullkl_r${LORA_RANK}_ridge_lam${lambda_name}_norm${normalization}"
  ...
  omnilens_add_run \
    ...
    "${output_dir}" \
    "${COMMON_RUN_KV[@]}" \
    "LORA_INIT_RIDGE_LAMBDA=${lambda}" \
    "LORA_INIT_NORMALIZATION=${normalization}"
}
...
echo "Preparing GPT-2 LoRA r=${LORA_RANK} ridge lambda/normalization ablation"
echo "Output root: ${BASE_OUT}"
echo "Lambdas: ${LAMBDAS[*]}"
echo "Normalizations: ${NORMALIZATIONS[*]}"
echo "Steps: ${NUM_STEPS}; save_every=${SAVE_EVERY}; save_initial=1"

for lambda in "${LAMBDAS[@]}"; do
  for normalization in "${NORMALIZATIONS[@]}"; do
    add_or_skip_run "${lambda}" "${normalization}"
  done
done

MANIFEST_FILE="${STAGING_DIR}/manifest.txt"
omnilens_write_manifest "${MANIFEST_FILE}"
