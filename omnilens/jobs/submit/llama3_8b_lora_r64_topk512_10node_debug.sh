...
RUN_ENV_INPUT="${1:-${REPO_DIR}/jobs/env/llama3_8b_lora_r64_subsetkl_topk512_10node_debug.env}"
...
PBS_JOBNAME="${PBS_JOBNAME:-llama3_8b_r64_expand_dbg}"

FORCED_BATCH_SIZE="${FORCED_BATCH_SIZE:-7}"
FORCED_SAVE_EVERY="${FORCED_SAVE_EVERY:-64}"
...
JOB_ID="$(
  qsub \
    ...
    -v "RUN_ENV_FILE=${STAGED_ENV_FILE},REPO_DIR=${REPO_DIR},OMNILENS_REPO_DIR=${REPO_DIR}" \
    "${PBS_SCRIPT}"
)"
