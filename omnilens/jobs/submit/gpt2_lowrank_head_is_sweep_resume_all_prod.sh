...
# Re-run the head/tail grid for low ranks, using the
# subset_kl importance-sampling estimator.
exec env \
  SWEEP_LABEL="GPT-2 low-rank head/tail IS sweep" \
  PART_INDEX=1 \
  PART_COUNT=1 \
  ...
  PBS_JOBNAME="${PBS_JOBNAME:-gpt2lowrank_is_prod}" \
