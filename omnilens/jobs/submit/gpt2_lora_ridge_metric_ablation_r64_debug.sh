...
PBS_WALLTIME="${PBS_WALLTIME:-00:45:00}"
PBS_FILESYSTEMS="${PBS_FILESYSTEMS:-YOUR_PBS_FILESYSTEMS}"
PBS_JOBNAME="${PBS_JOBNAME:-gpt2_ridge_metric_r64}"
MAX_CONCURRENT_RUNS="${MAX_CONCURRENT_RUNS:-2}"
DRY_RUN="${DRY_RUN:-0}"

BASE_OUT="${BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_lora_ridge_metric_ablation_r64_debug}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_lora_ridge_metric_ablation_r64_debug}"
LOG_SUBDIR="${LOG_SUBDIR:-gpt2_lora_ridge_metric_ablation_r64_debug}"
...
add_or_skip_run() {
  ...
  omnilens_add_run \
    ...
    "${output_dir}" \
    "${COMMON_RUN_KV[@]}" \
    "LORA_INIT_SVD_METRIC=${svd_metric}"
}
...
echo "Preparing GPT-2 LoRA r=${LORA_RANK} ridge metric ablation"
echo "Output root: ${BASE_OUT}"
echo "Steps: ${NUM_STEPS}; save_every=${SAVE_EVERY}; save_initial=1"

add_or_skip_run \
  "lora_fullkl_r${LORA_RANK}_ridge_residual_metric" \
  "${BASE_OUT}/lora_fullkl_r${LORA_RANK}_ridge_residual_metric" \
  "residual"

add_or_skip_run \
  "lora_fullkl_r${LORA_RANK}_ridge_unembed_metric" \
  "${BASE_OUT}/lora_fullkl_r${LORA_RANK}_ridge_unembed_metric" \
  "unembed"
