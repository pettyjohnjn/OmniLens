...
PBS_JOBNAME="${PBS_JOBNAME:-gpt2_expanded_hooks}"
MAX_CONCURRENT_RUNS="${MAX_CONCURRENT_RUNS:-1}"
DRY_RUN="${DRY_RUN:-0}"

BASE_OUT="${BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_expanded_hooks_three_run_1000}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_expanded_hooks_three_run_1000}"
LOG_SUBDIR="${LOG_SUBDIR:-gpt2_expanded_hooks_three_run_1000}"
...
echo "Preparing GPT-2 expanded-hook 3-run set"
echo "Output root: ${BASE_OUT}"
echo "Staging dir: ${STAGING_DIR}"

add_or_skip_run \
  "tuned_kl_baseline" \
  "${BASE_OUT}/tuned_kl_baseline" \
  "${COMMON_RUN_KV[@]}" \
  "LENS_TYPE=tuned" \
  "LOSS_TYPE=kl" \
...
add_or_skip_run \
  "lora_subset_topk_r${LORA_RANK}_k${TOPK_K}" \
  "${BASE_OUT}/lora_subset_topk_r${LORA_RANK}_k${TOPK_K}" \
  "${COMMON_RUN_KV[@]}" \
  "LENS_TYPE=lora" \
  "LOSS_TYPE=subset_kl" \
  "LORA_RANK=${LORA_RANK}" \
...
if (( TOTAL_RUNS == 0 )); then
  echo "No runnable expanded-hook runs were generated."
  exit 0
fi

MANIFEST_FILE="${STAGING_DIR}/manifest.gpt2_expanded_hooks.txt"
