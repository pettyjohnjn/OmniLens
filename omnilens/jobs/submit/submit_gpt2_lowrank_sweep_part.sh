...
BASE_OUT="${BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_debug_lowrank_sweep_1000}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_debug_lowrank_sweep_1000/part_${PART_INDEX}of${PART_COUNT}}"
LOG_SUBDIR="${LOG_SUBDIR:-gpt2_debug_lowrank_sweep_1000}"
...
HEAD_IS_MODE="${HEAD_IS_MODE:-is}"
HEAD_IS_RUN_FAMILY="${HEAD_IS_RUN_FAMILY:-${HEAD_IS_MODE}}"
...
INCLUDE_HEAD_TAIL="${INCLUDE_HEAD_TAIL:-1}"
...
add_head_is_runs() {
  ...
  for rank in "${LORA_RANKS[@]}"; do
    for spec in "${HEAD_TAIL_PAIRS[@]}"; do
      read -r k tail <<< "${spec}"
      add_or_skip_run \
        "lora_subset_${HEAD_IS_RUN_FAMILY}_r${rank}_k${k}_tail${tail}" \
        "${BASE_OUT}/lora_subset_${HEAD_IS_RUN_FAMILY}_r${rank}_k${k}_tail${tail}" \
        "${COMMON_RUN_KV[@]}" \
        "LENS_TYPE=lora" \
        "LOSS_TYPE=subset_kl" \
        "LORA_RANK=${rank}" \
        "SUBSET_KL_MODE=${HEAD_IS_MODE}" \
        "SUBSET_KL_K=${k}" \
...
echo "head+IS run family: ${HEAD_IS_RUN_FAMILY}"
echo "head+IS subset_kl mode: ${HEAD_IS_MODE}"
