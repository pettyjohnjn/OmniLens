...
PBS_JOBNAME="${PBS_JOBNAME:-gpt2_lora_init_fullkl}"
MAX_CONCURRENT_RUNS="${MAX_CONCURRENT_RUNS:-2}"
DRY_RUN="${DRY_RUN:-0}"

BASE_OUT="${BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_lora_init_fullkl_r64_debug}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_lora_init_fullkl_r64_debug}"
LOG_SUBDIR="${LOG_SUBDIR:-gpt2_lora_init_fullkl_r64_debug}"
...
echo "Preparing GPT-2 LoRA r=${LORA_RANK} full-KL init comparison"
...
add_or_skip_run \
  "lora_fullkl_r${LORA_RANK}_default_lora" \
  "${BASE_OUT}/lora_fullkl_r${LORA_RANK}_default_lora" \
  "${COMMON_RUN_KV[@]}" \
  "LORA_INIT=default_lora"

add_or_skip_run \
  "lora_fullkl_r${LORA_RANK}_ridge_svd" \
  "${BASE_OUT}/lora_fullkl_r${LORA_RANK}_ridge_svd" \
  "${COMMON_RUN_KV[@]}" \
  "LORA_INIT=ridge_svd" \
