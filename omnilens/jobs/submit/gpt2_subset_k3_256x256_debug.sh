...
PBS_JOBNAME="${PBS_JOBNAME:-gpt2_k3_256x256}"
MAX_CONCURRENT_RUNS="${MAX_CONCURRENT_RUNS:-1}"
DRY_RUN="${DRY_RUN:-0}"

BASE_OUT="${BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_subset_k3_256x256_debug}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_subset_k3_256x256_debug}"
LOG_SUBDIR="${LOG_SUBDIR:-gpt2_subset_k3_256x256_debug}"
...
echo "Preparing GPT-2 subset-KL K3 debug run"
echo "Output root: ${BASE_OUT}"
echo "Staging dir: ${STAGING_DIR}"
echo "Budget: head+IS ${HEAD_K}+${TAIL_K}"

add_or_skip_run \
  "lora_subset_k3_r${LORA_RANK}_k${HEAD_K}_tail${TAIL_K}" \
  "${BASE_OUT}/lora_subset_k3_r${LORA_RANK}_k${HEAD_K}_tail${TAIL_K}" \
  "${COMMON_RUN_KV[@]}" \
  "LENS_TYPE=lora" \
  "LOSS_TYPE=subset_kl" \
  "LORA_RANK=${LORA_RANK}" \
...
if (( TOTAL_RUNS == 0 )); then
  echo "No runnable K3 runs were generated."
  exit 0
fi

MANIFEST_FILE="${STAGING_DIR}/manifest.gpt2_subset_k3_256x256_debug.txt"
