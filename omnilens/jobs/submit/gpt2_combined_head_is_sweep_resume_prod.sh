...
PBS_JOBNAME="${PBS_JOBNAME:-gpt2_is_combined_prod}"
MAX_CONCURRENT_RUNS="${MAX_CONCURRENT_RUNS:-24}"
DRY_RUN="${DRY_RUN:-0}"

LOWRANK_BASE_OUT="${LOWRANK_BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_preemptable_lowrank_is_sweep_1000}"
STANDARD_BASE_OUT="${STANDARD_BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_preemptable_is_sweep_1000}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_combined_is_sweep_1000}"
...
add_rank_family() {
  ...
  for rank in "${ranks[@]}"; do
    for spec in "${HEAD_TAIL_PAIRS[@]}"; do
      read -r k tail <<< "${spec}"
      add_or_skip_run \
        "lora_subset_is_r${rank}_k${k}_tail${tail}" \
        "${base_out}/lora_subset_is_r${rank}_k${k}_tail${tail}" \
        "${log_subdir}" \
        "LENS_TYPE=lora" \
        "LOSS_TYPE=subset_kl" \
        "LORA_RANK=${rank}" \
...
echo "Preparing combined GPT-2 IS head/tail continuation"
echo "Low-rank root: ${LOWRANK_BASE_OUT}"
echo "Standard root: ${STANDARD_BASE_OUT}"
echo "Staging dir: ${STAGING_DIR}"

add_rank_family "${LOWRANK_BASE_OUT}" "gpt2_preemptable_lowrank_is_sweep_1000" "${LOWRANK_RANKS[@]}"
add_rank_family "${STANDARD_BASE_OUT}" "gpt2_preemptable_is_sweep_1000" "${STANDARD_RANKS[@]}"

TOTAL_RUNS="${#RUN_ENV_FILES[@]}"
if (( TOTAL_RUNS == 0 )); then
  echo "No runnable combined IS sweep runs were generated."
  exit 0
fi

MANIFEST_FILE="${STAGING_DIR}/manifest.gpt2_combined_is_sweep.txt"
