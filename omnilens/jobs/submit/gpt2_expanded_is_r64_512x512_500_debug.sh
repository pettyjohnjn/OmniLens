...
PBS_JOBNAME="${PBS_JOBNAME:-gpt2_expand_is_r64}"
DRY_RUN="${DRY_RUN:-0}"

RUN_NAME="${RUN_NAME:-lora_subset_is_gpt2_expanded_r64_k512_tail512_500}"
BASE_OUT="${BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_expanded_is_debug_500}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_expanded_is_debug_500}"
LOG_SUBDIR="${LOG_SUBDIR:-gpt2_expanded_is_debug_500}"
HF_HOME_DEFAULT="${HF_HOME_DEFAULT:-/path/to/hf_cache}"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  echo "DRY_RUN: qsub -q ${PBS_QUEUE} -A ${PBS_ACCOUNT} -N ${PBS_JOBNAME} -l select=${PBS_SELECT} -l walltime=${PBS_WALLTIME} -l filesystems=${PBS_FILESYSTEMS} -v RUN_ENV_FILE=${RUN_ENV_FILE},REPO_DIR=${REPO_DIR},OMNILENS_REPO_DIR=${REPO_DIR} ${PBS_SCRIPT}"
  exit 0
fi
...
qsub \
  ...
  -v "RUN_ENV_FILE=${RUN_ENV_FILE},REPO_DIR=${REPO_DIR},OMNILENS_REPO_DIR=${REPO_DIR}" \
  "${PBS_SCRIPT}"
