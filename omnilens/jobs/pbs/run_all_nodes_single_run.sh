...
resolve_repo_dir() {
  ...
  for candidate in "${OMNILENS_REPO_DIR:-}" "${REPO_DIR:-}" "${PBS_O_WORKDIR:-}"; do
...
if [[ -z "${REPO_DIR}" ]]; then
  echo "Unable to locate repo root; set REPO_DIR or OMNILENS_REPO_DIR before launching PBS."
  exit 1
fi
OMNILENS_REPO_DIR="${OMNILENS_REPO_DIR:-${REPO_DIR}}"
# shellcheck disable=SC1091
source "${REPO_DIR}/jobs/lib/common.sh"

TASK_SCRIPT="${TASK_SCRIPT:-${OMNILENS_REPO_DIR}/jobs/launch/run_env_task.sh}"
MASTER_PORT="${MASTER_PORT:-29500}"
NPROC_PER_NODE="${NPROC_PER_NODE:-4}"
REQUIRED_NNODES="${REQUIRED_NNODES:-0}"
INPUT_RUN_ENV_FILE="${RUN_ENV_FILE:-}"

cd "${OMNILENS_REPO_DIR}"
...
mapfile -t NODES < <(omnilens_unique_pbs_nodes)
NNODES="${#NODES[@]}"
...
if [[ -n "${INPUT_RUN_ENV_FILE}" ]]; then
  ...
  RUN_ENV_ROOT="${OMNILENS_REPO_DIR}/tmp/pbs_single_run_env"
  mkdir -p "${RUN_ENV_ROOT}"
  RUN_ENV_FILE="${RUN_ENV_ROOT}/$(date -u +%Y%m%dT%H%M%SZ).env"
  omnilens_write_current_env_file "${RUN_ENV_FILE}"
fi
