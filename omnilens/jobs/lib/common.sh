...
OMNILENS_REPO_DIR="${OMNILENS_REPO_DIR:-$(omnilens_repo_dir)}"
OMNILENS_SRC_DIR="${OMNILENS_SRC_DIR:-${OMNILENS_REPO_DIR}/src}"

omnilens_format_walltime() {
  local total_minutes="$1"
  printf '%02d:%02d:00' "$((total_minutes / 60))" "$((total_minutes % 60))"
}

omnilens_resolve_lora_alpha() {
  local rank="${1:-}"
...
omnilens_bootstrap_runtime() {
  local conda_env="${1:-${CONDA_ENV_NAME:-omnilens}}"
...
omnilens_install_runtime_deps() {
  ...
  # The three sub-packages ship with this repository under ../packages/, so the
  # default install sources are local paths requiring no network access.
  local packages_dir="${PACKAGES_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../packages" && pwd)}"
  local hookbox_source="${HOOKBOX_INSTALL_SOURCE:-${packages_dir}/hookbox}"
  local subset_source="${SUBSET_KL_INSTALL_SOURCE:-${packages_dir}/subset-kl}"
  local indexed_source="${INDEXED_LOGITS_INSTALL_SOURCE:-${packages_dir}/indexed_logits}"
...
for module_name in ("subset_kl", "omnilens"):
    try:
...
omnilens_write_current_env_file() {
  local env_file="$1"
  local name

  {
    while IFS= read -r name; do
      printf '%s=%q\n' "${name}" "${!name}"
    done < <(omnilens_exported_env_names)
  } > "${env_file}"
}
...
omnilens_write_manifest() {
  local manifest_file="$1"
  printf '%s\n' "${RUN_ENV_FILES[@]}" > "${manifest_file}"
}

omnilens_unique_pbs_nodes() {
