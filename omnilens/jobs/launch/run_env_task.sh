...
LOG_ROOT="${LOG_ROOT:-${OMNILENS_REPO_DIR}/logs/${LOG_SUBDIR}}"
SUBSET_KL_SRC_DIR="${SUBSET_KL_SRC_DIR:-${OMNILENS_REPO_DIR}/../subset-kl/src}"
...
export PYTHONPATH="${OMNILENS_SRC_DIR}:${PYTHONPATH:-}"
...
if [[ "${LENS_TYPE}" == "lora" ]]; then
  ...
  LORA_ALPHA="$(omnilens_resolve_lora_alpha "${LORA_RANK}" "${LORA_ALPHA-}")"
...
if [[ "${MEASURE_RSS}" == "1" ]]; then
  python "${OMNILENS_REPO_DIR}/scripts/run_with_rss_sampler.py" -- "${CMD[@]}"
else
  "${CMD[@]}"
fi
