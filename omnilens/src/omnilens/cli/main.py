# src/omnilens/cli/main.py
"""
Command-line interface for OmniLens training.
...
from omnilens.initialization import LoRAInitConfig, initialize_lowrank_lens
from omnilens.lenses import LowRankLens, create_lens
from omnilens.losses import create_loss
from omnilens.training import (
    LensTrainer,
    TrainConfig,
    HFUnembed,
    get_model_config,
...
def build_dataloader_factory(config: TrainConfig, tokenizer, ddp_state: DDPState):
    ...
    def factory() -> DataLoader:
        ...
        if config.data_source == "text":
            ...
            # Pile source
            from omnilens.data import PileSource, PileSourceConfig
            pile_cfg = PileSourceConfig(split=config.pile_split)
            pile_source = PileSource(pile_cfg)
            from omnilens.data import StreamingTextSource
...
def train(args: argparse.Namespace) -> None:
    ...
    config = TrainConfig(
        ...
        stream_load=getattr(args, "stream_load", False),
    )
    ...
    try:
        ...
        if config.multinode_model_parallel and config.stream_load:
            # Path 1b: Multi-node FSDP with streaming load. Never materializes the
            # full model on any node's CPU -- required for models larger than node
            # RAM (405B = 812GB > 512GB/node). Meta-init, stream unembed + params
            # from disk during FSDP wrap. See training/stream_load.py.
            from ..training.stream_load import (
                build_meta_model,
                materialize_params,
                wrap_fsdp_streaming,
            )

            meta_model, _st_index, _st_qnames = build_meta_model(
                config.model_name, model_dtype,
                attn_implementation=config.attn_implementation,
            )
            model_cfg = get_model_config(meta_model)  # config only, no weights
            activation_site_plan = build_activation_site_plan(
                meta_model,
                num_layers=model_cfg["num_layers"],
                preset=config.activation_site_preset,
            )
            # Materialize just norm + lm_head so the unembed has real weights.
            materialize_params(
                meta_model, _st_index,
                ["model.norm.weight", "lm_head.weight", "model.embed_tokens.weight"],
                ddp_state.device, model_dtype,
            )
            unembed = HFUnembed(meta_model).clone_to_device(ddp_state.device)
            logger.info(f"Cloned unembed to {ddp_state.device} (streaming, pre-FSDP)")

            model, _mn_shard = wrap_fsdp_streaming(
                meta_model, _st_index, _st_qnames,
            ...
            activation_site_plan = adapt_activation_site_plan_for_model(
                model, activation_site_plan,
            )
            shard_state = disabled_shard_state(ddp_state.device)
            lens_device = ddp_state.device
            if ddp_state.is_main:
                logger.info(f"FSDP sharding (streaming): {_mn_shard}")

        elif config.multinode_model_parallel:
        ...
        if config.lora_init != "default_lora":
            if config.lens_type != "lora" or not isinstance(lens, LowRankLens):
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="OmniLens - Scalable lens-based interpretability",
    )
    ...
    train_parser.add_argument(
        "--lens_type",
        choices=["logit", "tuned", "lora"],
        default="lora",
        help="lora: low-rank translator (default). tuned: full-rank affine. logit: frozen readout.",
    )
    ...
    train_parser.add_argument(
        "--subset_kl_mode",
        choices=["topk", "is", "k2", "k3"],
        default="topk",
        help=(
            "Subset KL estimator. "
            "topk: renormalize KL over top-k tokens only — fast, recommended default. "
            "is: exact KL on top-k head + importance-weighted estimate on the tail. "
            "k2: top-k head KL + Schulman K2 squared-error tail penalty. "
            "k3: top-k head KL + Schulman K3 tail estimator. "
            "Use --subset_kl_k_tail > 0 to enable the tail term for is/k2/k3."
        ),
    )
    train_parser.add_argument("--subset_kl_k_tail", type=int, default=0,
                              help="Number of tail samples per token for is/k2/k3 modes (0 = head only)")
    train_parser.add_argument(
        "--subset_kl_tail_proposal",
        choices=["target", "teacher", "mixed", "tempered"],
        default="target",
        help="Proposal distribution for tail sampling in is/k2/k3 modes. target uses the teacher distribution.",
    )
    ...
    # Accepted for launcher compatibility (jobs/launch/run_env_task.sh passes these
    # for every subset_kl run). Not currently consumed by _build_loss / the loss;
    # kept so the parser doesn't reject them. Wire into the loss when tail
    # importance-sampling gains clip/oversample support.
    train_parser.add_argument("--subset_kl_tail_clip", type=float, default=50.0,
                              help="(reserved) importance-weight clip for is-tail proposals")
    train_parser.add_argument("--subset_kl_tail_oversample", type=int, default=4,
                              help="(reserved) oversampling factor for is-tail proposals")
    ...
    train_parser.add_argument("--stream_load", action="store_true", default=False,
                              help="Stream weights from disk during FSDP wrap (rank 0 reads "
                                   "one layer at a time) instead of loading the full model to "
                                   "CPU on every rank. Required for models larger than node RAM.")
