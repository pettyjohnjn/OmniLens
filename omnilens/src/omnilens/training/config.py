# src/omnilens/training/config.py
...
class TrainConfig:
    ...
    subset_kl_mode: Literal["topk", "is", "k2", "k3"] = "topk"
    ...
    # Stream weights from disk during FSDP wrap instead of loading the full model
    # to CPU on every rank. Required for models too large for one node's RAM
    # (e.g. 405B = 812GB > 512GB/node). No effect unless multinode_model_parallel.
    ...
    def canonical_run_path(self) -> Path:
        ...
        where ``{init}`` is omitted for non-low-rank lens types and ``{tag}`` is
