# src/omnilens/lenses/lowrank_lens.py
"""
OmniLens - Parameter-efficient lens using low-rank adaptation.
...
- LowRankLens: stores d×r + r×d per layer = O(L * 2dr) parameters

For d=4096, r=16: 16M vs 131K per layer = 128x reduction

Paper mapping: implements the low-rank translator of Sec. 3
L = I + (alpha/r) B A plus bias b -- a rank-at-most-r update
to the identity; A is Xavier-initialized, B and b start at zero so
training begins from the identity map (Sec. 3, 'Low-rank translator').
"""
...
from .streaming_lse import streaming_logsumexp
...
class LowRankProjection(nn.Module):
    ...
    def __repr__(self) -> str:
        return f"LowRankProjection(hidden={self.hidden_size}, r={self.r}, alpha={self.alpha})"


class LowRankLens(BaseLens):
    """
    OmniLens: parameter-efficient per-layer projections.
    ...
    - LowRankLens: 12 * 2 * 768 * 16 = 295K trainable params (24x reduction)

    For LLaMA-70B (d=8192, L=80, r=16):
    - TunedLens: 80 * 8192² = 5.4B trainable params (!)
    - LowRankLens: 80 * 2 * 8192 * 16 = 21M trainable params (256x reduction)
    """
    ...
    def __init__(
        ...
        self.projections = nn.ModuleDict({
            self._module_keys[lid]: LowRankProjection(
    ...
    def compute_logits(
        ...
        if layer is None:
            raise ValueError("LowRankLens requires a layer argument.")
    ...
    def compute_logits_subset_with_logsumexp(
        ...
        Memory: one low-precision [N, V] logits tensor; the fp32 log-partition
        streams in vocab chunks (streaming_lse) rather than materializing an
        fp32 copy. Fine with layer-wise backward.
        """
        if layer is None:
            raise ValueError("LowRankLens requires a layer argument.")
        ...
        logsumexp = streaming_logsumexp(full_logits)  # [N, 1] fp32, no full-vocab fp32 copy
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"LowRankLens(vocab_size={self.vocab_size}, r={self.r}, "
