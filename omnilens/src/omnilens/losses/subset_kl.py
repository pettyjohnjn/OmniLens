# src/omnilens/losses/subset_kl.py
"""
...
head/tail estimators conforming to the omnilens ``BaseLoss`` interface.

Four modes are supported:
- ``"topk"`` (default): delegates to ``subset_kl.SubsetKLLoss`` for
  simple top-k renormalized KL — recommended for most training runs.
- ``"is"``: exact head KL plus importance-sampled tail estimate.
...
if TYPE_CHECKING:
    from omnilens.lenses import BaseLens


SubsetMode = Literal["topk", "is", "k2", "k3"]
...
def _prepare_k2_teacher_subset(
    ...
    with torch.no_grad():
        # One full-vocabulary fp32 buffer, transformed in place
        # (logits -> log-probs -> tail probs -> proposal); the old
        # out-of-place chain held up to five [N, V] tensors at once.
        work = teacher_logits.to(torch.float32, copy=True)
        work.sub_(work.logsumexp(dim=-1, keepdim=True))  # log-probs

        if k_head > 0:
            # topk on log-probs selects the same indices as on logits
            head_log_probs, head_idx = work.topk(k_head, dim=-1)
            head_logprobs_k = head_log_probs - torch.logsumexp(
                head_log_probs, dim=-1, keepdim=True
            )
            head_probs_k = head_logprobs_k.exp()
            p_head = head_log_probs.exp().sum(dim=-1)
        else:
        ...
        if k_tail > 0:
            ...
            work.exp_()                                   # probs
            if k_head > 0:
                work.scatter_(1, head_idx, 0.0)           # tail probs
            tail_mass = work.sum(dim=-1, keepdim=True).clamp_min(1e-12)
            if tail_proposal in ("target", "teacher"):
                work.div_(tail_mass)                      # proposal, in place
                tail_idx = torch.multinomial(
                    work, num_samples=k_tail, replacement=True
                )
                tail_proposal_log_probs = torch.gather(
                    work, 1, tail_idx
                ).clamp_min(1e-45).log()
                # true tail log-prob: log(q * mass) = log q + log mass
                tail_log_probs = tail_proposal_log_probs + tail_mass.log()
            else:
                ...
                # work still holds true tail probs (head entries zeroed only)
                tail_log_probs = torch.gather(
                    work, 1, tail_idx
                ).clamp_min(1e-45).log()
                tail_proposal_log_probs = torch.gather(
...
def _is_tail_kl(
    ...
    """Exact head KL plus an importance-sampled tail estimate."""
    if k_head > 0:
...
class SubsetKLLoss(BaseLoss):
    ...
    mode : str
        ...
        ``"is"``: exact head KL + importance-weighted tail estimate.
    ...
    k_tail : int
        (``"is"``/``"k2"``/``"k3"`` modes) Number of tail samples. Default 0
        disables the tail term and behaves like ``"topk"`` for those modes.
    tail_proposal : str
        Proposal distribution for tail sampling in ``"is"``/``"k2"``/``"k3"``
    ...
    IS estimator with tail correction:

    >>> loss_fn = SubsetKLLoss(k=128, mode="is", k_tail=64)
    """
    ...
    def forward_with_lens(
        ...
        if subset["mode"] in ("is",):
            return self._forward_with_lens_is(hidden_states, lens, layer, attention_mask, subset)
