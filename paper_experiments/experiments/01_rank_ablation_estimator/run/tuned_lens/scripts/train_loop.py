...
def _all_reduce_sum(x: th.Tensor) -> th.Tensor:
...
def _all_reduce_mean(x: th.Tensor) -> th.Tensor:
    if not _dist_is_init():
        return x
    y = x.clone()
    dist.all_reduce(y, op=dist.ReduceOp.SUM)
...
def _gather_vec(x: th.Tensor) -> list[th.Tensor]:
    if not _dist_is_init():
        return [x]
    outs = [th.zeros_like(x) for _ in range(dist.get_world_size())]
...
class Train:
    ...
    def execute(self):
        ...
        step_tokens_accum = 0.0
        step_loss_accum = 0.0
        step_loss_count = 0  # number of layer-losses accumulated (layers * microbatches)
        step_t0 = None
        ...
        for batch_idx, batch in zip(t, state.dataloader):
            ...
            # Start-of-optimizer-step bookkeeping
            ...
            if rem == 0:
                ...
                if self.dist.device.type == "cuda":
                    th.cuda.reset_peak_memory_stats(self.dist.device)
                    th.cuda.synchronize(self.dist.device)
            ...
            # Token counting for throughput (use attention_mask if available)
            if "attention_mask" in batch:
                toks_local = batch["attention_mask"].sum()
            else:
                toks_local = th.tensor(batch["input_ids"].numel(), device=self.dist.device)

            toks_local = toks_local.to(dtype=th.float32)
            toks_global = _all_reduce_sum(toks_local)
            step_tokens_accum += float(toks_global.item())
            ...
            for i, h in enumerate(hidden_states):
                with th.autocast(self.dist.device.type, dtype=th.bfloat16):
                    ...
                    # For 1:1 comparable logging: accumulate mean loss over layers,
                    # averaged across ranks.
                    loss_mean = _all_reduce_mean(loss.detach().float())
                    step_loss_accum += float(loss_mean.item())
                    step_loss_count += 1
            ...
            if rem == grad_acc_steps - 1:
                ...
                # End timing for ONE tuned-lens optimizer step
                dt = time.perf_counter() - float(step_t0)

                # Update global token counter
                total_tokens_seen += step_tokens_accum

                # Compute comparable metrics
                mean_nats = step_loss_accum / max(1, step_loss_count)
                loss_bpb = mean_nats * state.nats_to_bpb
                tok_per_s = step_tokens_accum / max(dt, 1e-9)

                # LR
                lr = state.opt.param_groups[0]["lr"]

                # Peak memory per rank (for this step)
                mem_line = ""
                if self.dist.device.type == "cuda":
                    peak_alloc = float(th.cuda.max_memory_allocated(self.dist.device))
                    peak_resv = float(th.cuda.max_memory_reserved(self.dist.device))
                    mem_vec = th.tensor([peak_alloc, peak_resv], device=self.dist.device, dtype=th.float64)
                    gathered = _gather_vec(mem_vec)
                ...
                # Print exactly like LowRankLens loop
                if self.dist.primary:
                    logger.info(
                        f"[Train] step {state.step + 1}/{self.num_steps} "
                        f"loss={loss_bpb:.4f} (bits/byte) "
                        f"mean_nats={mean_nats:.4f} "
                        f"tok/s={tok_per_s:.0f} "
                        f"dt={dt*1000:.1f}ms "
                        f"tokens={int(total_tokens_seen)} "
                        f"lr={lr:.2e}"
                        f"{mem_line}"
                    )
