...
LR_VALUES_CSV="${LR_VALUES_CSV:-2e-4,5e-4,1e-3,1.5e-3,2e-3}"
IFS=',' read -r -a LR_VALUES <<< "${LR_VALUES_CSV}"
MODE_VALUES=(k2 k3)
RUN_COUNT_TARGET="$(( ${#LR_VALUES[@]} * ${#MODE_VALUES[@]} ))"
...
PBS_JOBNAME="${PBS_JOBNAME:-gpt2_k2k3_lr}"
MAX_CONCURRENT_RUNS="${MAX_CONCURRENT_RUNS:-10}"
DRY_RUN="${DRY_RUN:-0}"

BASE_OUT="${BASE_OUT:-${REPO_DIR}/src/checkpoints/gpt2_subset_k2_k3_lr_sweep_250_debug}"
STAGING_ROOT="${STAGING_ROOT:-${REPO_DIR}/tmp/gpt2_subset_k2_k3_lr_sweep_250_debug}"
LOG_SUBDIR="${LOG_SUBDIR:-gpt2_subset_k2_k3_lr_sweep_250_debug}"
...
echo "Preparing GPT-2 subset-KL K2/K3 LR sweep"
echo "Output root: ${BASE_OUT}"
echo "Staging dir: ${STAGING_DIR}"
echo "LR values: ${LR_VALUES[*]}"
echo "Modes: ${MODE_VALUES[*]}"
echo "Budget: head+IS ${HEAD_K}+${TAIL_K}"
echo "Requested runs: ${RUN_COUNT_TARGET}"

for mode in "${MODE_VALUES[@]}"; do
  for lr in "${LR_VALUES[@]}"; do
    slug="$(lr_slug "${lr}")"
    add_or_skip_run \
      "lora_subset_${mode}_r${LORA_RANK}_k${HEAD_K}_tail${TAIL_K}_lr${slug}" \
      "${BASE_OUT}/lora_subset_${mode}_r${LORA_RANK}_k${HEAD_K}_tail${TAIL_K}_lr${slug}" \
      "${COMMON_RUN_KV[@]}" \
      "LR=${lr}" \
      "LENS_TYPE=lora" \
      "LOSS_TYPE=subset_kl" \
...
if (( TOTAL_RUNS == 0 )); then
  echo "No runnable K2/K3 LR-sweep runs were generated."
  exit 0
fi

MANIFEST_FILE="${STAGING_DIR}/manifest.gpt2_subset_k2_k3_lr_sweep_250_debug.txt"
